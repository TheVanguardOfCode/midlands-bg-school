...
import { LinkItem, DropdownLink } from "./model/nav-data.types";
import { html, render, TemplateResult, page } from "./lib";
import { getNavData } from "./utils/nav-data-util";
import { initTheme } from "./theme-switcher/theme";
import { initI18n } from "./i18n/i18n";
import { locales } from "./utils/i18n-util";
import { homeView } from "./views/home-view";
import { headerTemplate } from "./templates/header-template";
import { navMenuTemplate } from "./templates/nav-menu-template";
import { navLinkTemplate } from "./templates/nav-link";
import { navDropdownTemplate } from "./templates/nav-dropdown";
import { navDropdownLinkTemplate } from "./templates/nav-dropdown-link";
...
window.addEventListener("DOMContentLoaded", async (): Promise<void> => {
    ...
    const links = navData.links.map((link: LinkItem) => {
        ...
        if (link.isDropdown && link.dropdownLinks) {
            ...
            const currentDropdownlinksTemplate = html`
    ...
    const navMenuList: TemplateResult = html` ${links}`;
    const navMenu: TemplateResult = navMenuTemplate(navMenuList);
